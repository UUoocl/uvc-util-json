...
//
  ...
  - (NSDictionary*) summaryDictionary
  {
    ...
    if ( [self hasRange] ) {
      if (_minimum) [dict setObject:[_minimum jsonObject] forKey:@"minimum"];
      if (_maximum) [dict setObject:[_maximum jsonObject] forKey:@"maximum"];
    }
    if ( [self hasStepSize] ) {
      if (_stepSize) [dict setObject:[_stepSize jsonObject] forKey:@"step-size"];
    }
    if ( [self hasDefaultValue] ) {
      if (_defaultValue) [dict setObject:[_defaultValue jsonObject] forKey:@"default-value"];
    }
    
    UVCValue    *curValue = [self currentValue];
    if ( curValue ) [dict setObject:[curValue jsonObject] forKey:@"current-value"];
